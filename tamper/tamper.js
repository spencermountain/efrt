...
Tamper.unpackExistence = function(element, default_attrs) {
  ...
  var bitArray = Tamper.biterate(element.pack),
    ...
    consumeCC = function(array) {
      if (array.length < 2) {
        throw 'Improperly formatted bit array'
      }
    ...
    consumeChunk = function(bytes, bits, array) {
      ...
      while (i < num_bits) {
        if (!!chunk[i]) {
          ...
          output.push(_.extend(default_attrs, {
            guid: counter
          }));
        }
        counter++;
...
Tamper.unpackData = function(data, default_attrs) {
  if (typeof (default_attrs) === 'undefined') {
    var default_attrs = {};
  }
  if (data.existence) {
    var exists = Tamper.unpackExistence(data.existence, default_attrs),
      collection;
    _(data.attributes).each(function(a) {
      ...
      _(exists).each(function(s, i) {
        s[a.attr_name] = attr_array[i];
      });
    });
    return exists;
  } else {
    seeds = _(data.collection).map(function(i) {
      var default_attrs = _.clone(default_attrs);
      return _(default_attrs).extend(i);
    });
    return seeds;
  }
};
