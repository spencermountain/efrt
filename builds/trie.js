...
},{"./trie":4}],3:[function(_dereq_,module,exports){
'use strict';
...
var toAlphaCode = function toAlphaCode(n) {
  ...
  for (places = 1, range = BASE; n >= range; n -= range, places++, range *= BASE) {}
...
var fromAlphaCode = function fromAlphaCode(s) {
  ...
  for (places = 1, range = BASE; places < s.length; n += range, places++, range *= BASE) {}
...
var Trie = function () {
  ...
  _createClass(Trie, [{
    ...
    value: function _insert(word, node) {
      ...
      for (var i = 0; i < keys.length; i++) {
        var prop = keys[i];
        var prefix = commonPrefix(word, prop);
        if (prefix.length === 0) {
          continue;
        }
    ...
    value: function pack() {
      ...
      function symbolCount() {
        ...
        for (var sym = 0; sym < ptrie.BASE; sym++) {
          ...
          savings[sym] = histAbs[sym][1] - defSize - histRel.countOf(ptrie.BASE - sym - 1) + savings[sym - 1];
      ...
      for (var _sym = symCount - 1; _sym >= 0; _sym--) {
        nodes.unshift(ptrie.toAlphaCode(_sym) + ':' + ptrie.toAlphaCode(nodeCount - histAbs[_sym][0] - 1));
      }
